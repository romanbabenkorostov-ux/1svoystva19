...
def mode_predict():
    if IN_STREAMLIT:
        st.markdown("**🌱 Прогноз по действиям**")
    else:
...
def mode_recommend():
    if IN_STREAMLIT:
        st.markdown("**🎯 Рекомендация по целям**")
    else:
...
def run_streamlit():
    ...
    # Заголовок меньшим шрифтом
    st.markdown("#### 🌱 Моделирование свойств почв по реакции на агротехнические и биотехнологические воздействия")
    ...
    # Выбор режима меньшим шрифтом
    st.markdown("**Режим:**")
    mode = st.radio("Режим:", ["Прогноз по действиям", "Рекомендация по целям"], horizontal=True, label_visibility="collapsed")
